...
| `bTOAP` | Create Client | POST | `/customers` | 1 | 顧客オブジェクト作成（**重複定義。account_createと同一エンドポイント**） |
...
API定義は計15本。うち実使用は**10本**、未使用が**4本**（Get Client / Create Client / Create Token / Charge）、エラー用1本。
...
| `bTHef` | payjp_webhook_update_**talent** | `body.type == "tenant.updated"` | Webhook_Eventレコード作成 + Userの`reviewed_brand`更新。審査結果通知時に発火 |
