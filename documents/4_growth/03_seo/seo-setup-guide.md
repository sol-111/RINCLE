...
### RINCLEの状態と権限の仕組み（2026-08-26時点）

プロパティ `rincle.co.jp`（ドメインプロパティ）は登録・所有者確認済み（所有者は増永さん側のアカウント —「設定 > ユーザーと権限」で確認できる）。弊社は権限付与ユーザーとしてアクセス可能のため、プロパティ作成の作業は不要だった。
...
✅ **2026-08-26 送信済み: 「成功しました」・検出408ページ。** 今後sitemapを作り直した時は、Bubbleに再アップロードするだけでよい（Googleが定期的に再読込する。急ぐ時だけ再送信）。
...
✅ **2026-08-26 下記10件のリクエスト完了。** 対象（静的2+在庫の多い店舗8）。店舗ページは「尾道 ロードバイク レンタル」のような地域検索の受け皿になるため、在庫が多く中身が濃い順に選定:
