...
> **2026-08-26更新: プロパティは既に存在していた。** `rincle.co.jp` のドメインプロパティが登録済みで、
> 所有者確認も完了している（所有者は増永さん側のアカウントと思われる —「設定 > ユーザーと権限」で確認できる）。
> 清野は「所有者から権限を付与されたユーザー」としてアクセス可能。
> したがって下の1〜3（プロパティ作成・所有権確認）は**不要**。旧手順として残す。

**0. 自分の権限を確認（まずこれ）**
- 設定 →「ユーザーと権限」で自分の権限レベルを見る
- 「フル」ならsitemap送信・URL検査まで一通りできる。「制限付き」なら閲覧のみ → 所有者にフルへの変更を頼む
- レポート自動化用のロボット（サービスアカウント）の追加は**所有者のみ可能** → 所有者に「制限付きで追加」を依頼する

<details>
<summary>旧手順（プロパティ新規作成・所有権確認 — 既に済んでいるため通常は不要）</summary>
