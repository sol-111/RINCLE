...
Googleの検索結果で見ると:
  ...
  全国のプロショップでロードバイク・E-Bikeをレンタル。                          ← これがdescription
```
...
|--------|---------------------|
| トップページ | 全国のプロショップでロードバイク・E-Bikeをレンタル。プロの整備で安心、ネットで簡単予約。初めてのスポーツバイク体験にも最適です。 |
| 店舗一覧 | RINCLEのパートナーショップを検索。全国のプロショップから、最寄りの店舗を見つけてロードバイク・E-Bikeをレンタルできます。 |
| 店舗詳細（例: 神戸） | スペシャライズド神戸でロードバイク・E-Bikeをレンタル。JR神戸駅から徒歩3分。プロスタッフが初心者にも丁寧にサポートします。 |

**ルール:**
- 70〜120文字が目安
- そのページに何が書いてあるかを具体的に書く
- 「全国のプロショップ」等の差別化ポイントを入れる（店舗数の対外表記は「全国100店舗以上」で決め打ちする方向で協議中〈6月19日・タスク#33・増永さん側で決定〉。確定後に例文へ反映する）
...
<meta property="og:description" content="全国のプロショップでロードバイク・E-Bikeをレンタル。">
...
## Step 3: Google Search Console — Googleとの窓口

### Search Consoleとは

**Googleが無料で提供する「あなたのサイトがGoogleにどう見えているか」を確認・操作できる管理画面。**
SEOの効果測定は、最終的にすべてこの画面の数字で答え合わせする。

GA4との役割分担:
- **Search Console** = 検索結果に**「出るまで」**を見る（何ページ登録されたか・どんな検索語で何回表示・何位・何回クリック）
- **GA4**（Step 4）= サイトに**「来た後」**を見る（何人来たか・どこから来たか・どのページを見て予約したか）

### 仕組み: 検索結果に載るまでの3段階

▼ クロール
Googleのロボット（Googlebot）がページを読みに来ること
ロボットは「知っているURL」しか読みに来ない — だからsitemapでURL一覧を渡す（Step 2）

▼ インデックス
読んだページがGoogleのデータベースに登録されること
**登録されていないページは、何を検索しても出てこない**。クロール → インデックス → 検索結果に表示、の順

RINCLEは2026年6月の診断でインデックスが**2〜3ページ**しかなかった（Googleがページを見つけられていない「ディスカバリー問題」）。sitemapとこのStepの作業はその解決策そのもの。

### プロパティの導入（初回のみ）

新しいドメインやサブドメインをSearch Consoleで見たい時にやる。プロパティは2種類あり、確認方法が違う:

| 種類 | カバー範囲 | 所有権の確認方法 |
|---|---|---|
| ドメイン | サブドメイン・http/httpsを丸ごと（例: `rincle.co.jp` 全体） | **DNSのTXTレコードのみ**（DNS設定を触れる人の作業） |
| URLプレフィックス | そのURL配下だけ（例: `https://rincle.co.jp/`） | HTMLタグ・HTMLファイル・GA4連携など**選択肢が多い**。Bubbleなら下の手順が簡単 |

URLプレフィックスをBubbleで確認する手順:
1. Search Consoleにログイン →「プロパティを追加」→「URLプレフィックス」を選び `https://rincle.co.jp/` を入力
2. 確認方法で「HTMLタグ」を選択し、表示されたmetaタグ（`<meta name="google-site-verification" ...>`）をコピー
3. Bubble.io Settings → SEO/metatags →「Script/meta tags in header」に貼り付けて保存 → 本番デプロイ
4. Search Consoleに戻り「確認」をクリック。「所有権を確認しました」が出ればOK

- 「HTMLファイル」方式を選ぶ場合は、確認用ファイルをsitemapと同じ「Hosting files in the root directory」に置けばよい
- 既にあるプロパティに人を招く時は、所有者が「設定 → ユーザーと権限 → ユーザーを追加」（プロパティを作り直す必要はない）

### 権限の仕組み

| 権限 | できること |
|---|---|
| 所有者 | すべての操作+**ユーザーやロボットの追加・削除**（これだけは所有者限定） |
| フル | データ閲覧+sitemap送信・URL検査などの操作 |
| 制限付き | データの閲覧のみ |

- 月次レポート自動化用のロボット（サービスアカウント）の追加は所有者のみ可能 → 所有者に「制限付きで追加」を依頼する

### sitemapの送信

なぜ「送信」するのか:
- sitemap.xmlをサイトに置いただけだと、Googleが自力で気づくのを待つことになる。送信すれば「ここに一覧表を置いた」と直接知らせられる（確実で速い）
- 送信すると**読み込みの結果が画面で見える**ようになる — 何ページ検出されたか・エラーが無いか。置くだけでは確認手段がない

方法:
1. 左メニュー「サイトマップ」を開く
2. 「新しいサイトマップの追加」に `https://rincle.co.jp/sitemap.xml` を入力（ドメインプロパティなので完全なURLで）
3. 「送信」→ ステータスが「成功しました」になればOK（送信直後は「取得できませんでした」と出ることがあるが、Googleが再取得して数時間〜1日で直る）

### URL検査 — 主要ページの「割り込み申請」

なぜやるのか:
- sitemapは「一覧を渡す」だけで、いつ読みに来るかはGoogle任せ（数日〜数週間かかる）
- URL検査から「インデックス登録をリクエスト」すると、そのページを**優先的に見てもらえる** — 行列への割り込み申請のようなもので、立ち上がりが速くなる
- 1日にリクエストできる件数には上限がある（10件ほど）ので、**価値の高いページから**使う

方法: 画面上部の検索バーにURLを貼る → 検査結果が出たら「**インデックス登録をリクエスト**」ボタン（1件1〜2分かかる）。

対象の選び方: 静的ページ+**在庫が多く中身の濃い店舗ページ**。選定例（2026-08-26・在庫数順）。店舗ページは「尾道 ロードバイク レンタル」のような地域検索の受け皿になるため、在庫が多く中身が濃い順に選定:

| ページ | URL |
|---|---|
| トップ | `https://rincle.co.jp/` |
| 検索 | `https://rincle.co.jp/search` |
| 広島尾道（24台） | `https://rincle.co.jp/shop_detail?shop=1784201033377x573162146841333060` |
| 尼崎西昆陽（14台） | `https://rincle.co.jp/shop_detail?shop=1784201041101x118677463101759970` |
| 茨城つくば（12台） | `https://rincle.co.jp/shop_detail?shop=1784201029340x628753700399960600` |
| 大阪南河内（12台） | `https://rincle.co.jp/shop_detail?shop=1784201030358x739738315518292000` |
| 熊本駅（12台） | `https://rincle.co.jp/shop_detail?shop=1784201031477x479744496774088500` |
| 熊本南（12台） | `https://rincle.co.jp/shop_detail?shop=1784201037190x235666585127260130` |
| 京都西京極駅（12台） | `https://rincle.co.jp/shop_detail?shop=1784201040957x106611676762389920` |
| 愛媛松山市駅（11台） | `https://rincle.co.jp/shop_detail?shop=1784201032411x680341710859929700` |

- 自転車詳細（342ページ）は個別リクエストせず、sitemapからの自然な巡回に任せる（上限枠は店舗ページに使う）

### 日々どこを見るか

| 左メニュー | 見るもの | 何がわかる |
|---|---|---|
| ページ | インデックス登録済みの数 | 408ページ中いくつ検索に載ったか — sitemap効果の答え合わせ。6月時点の2〜3ページからの伸びを見る |
| 検索パフォーマンス | 表示回数・クリック数・平均掲載順位・検索語 | どんな言葉で検索されて出ているか — 「ロードバイク軸」のKW戦略の実測 |
| サイトマップ | ステータス・検出ページ数 | sitemapが正しく読まれ続けているか。再生成後の反映確認もここ |

- インデックス数・表示回数・クリック数は月次レポートに自動掲載予定（ロボットの追加が済み次第、実装）
