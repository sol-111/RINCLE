...
2. 借りられないもの・存在しないものを除外
   - 店舗: 削除済み（`deleted_at`）・審査未通過（`brand_status` が passed 以外）
   - 自転車: 削除済み・アーカイブ済み（`__is_archive`）・ユーザー非表示（`rental_status`）・**掲載しない店舗（審査未通過や削除済み）に属するもの**（予約できないため）
   - **在庫ゼロの店舗は載せる** — 「レンタル車体準備中」の店舗もページを登録する。検索は**ページの歴（登録されてからの期間）が効く**ので、入荷を待たずにURLを育てておく（2026-08-21 清野さん判断）
3. 静的3ページ + 店舗 + 自転車のURLを並べたXMLを出力

本番反映前に動きを試したい時は `--dev` を付けると開発版（version-test）のData APIで動く
**生成後の確認**: 出力された件数を確かめる（2026-08-21の初回生成は 静的3 + 店舗63 + 自転車342 = **408 URL**。店舗は全67件中、審査未通過3・辞退1を除外／自転車は全440件中、非表示89〈うち削除済み6〉と掲載しない店舗の9台を除外＝440−89−9）
- 店舗はサイトの検索に出る数より多くなるのが正常（在庫ゼロの店舗も載せているため。2026-08-21時点で sitemap 63件 / 検索表示 53件）
- 自転車はサイトの検索に出る数とほぼ一致するはず。大きくズレたら除外条件（非表示・削除・アーカイブの判定）が実態と合っていないので要調査
...
sitemapのほとんどは店舗・自転車のURLなので、増えるたびに作り直す価値がある。

**手順3（再生成） → 手順4（再アップロード） → Search Consoleが自動で読み直す**

- 手順1・2は最初の1回だけ（ページの種類を増やした時のみ再確認）
- 再生成の頻度（月1回など）は増永さんと相談して決める
