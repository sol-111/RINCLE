// OS_DATA から flat な option_items レコードを生成
const OS_DATA = [
  {name:'Rights', attrs:[],
...
for (const os of OS_DATA) {
  // ヘッダー行（セット名）
  const attrLabels = os.attrs.map(a => a.label).join(', ');
  flatRows.push({
    sort_order: sortOrder++,
    set_name: os.name,
    notes: os.notes || '',
    abbrev: os.abbrev || '',
    display: attrLabels,  // attr labels をdisplay列に入れる（ヘッダー行の目印）
    val_1: '', val_2: '', val_3: '',
  });
...
writeFileSync('scripts/seed_os_v2.sql', sql);
console.log(`✓ scripts/seed_os_v2.sql を生成しました (${flatRows.length}行)`);
