...
async function freshen(page) {
  // Bubble「アプリが更新されました」バナーが出ていたらリロード（ワークフロー凍結解除）
  const stale = await page.getByText('アプリが更新されました').first().isVisible().catch(() => false);
  if (stale) {
    console.log('!! stale-app banner detected -> reload');
...
function dump() {
  ...
  const inputs = Array.from(document.querySelectorAll('input')).map(i => ({ type: i.type, vis: i.getBoundingClientRect().width > 0, ph: i.placeholder, ac: i.getAttribute('autocomplete') })).filter(x => x.vis);
  const radios = Array.from(document.querySelectorAll('input[type=radio]')).map(r => ({ vis: r.getBoundingClientRect().width > 0, near: (r.closest('.bubble-element')?.textContent || '').trim().slice(0, 20), checked: r.checked }));
  const set = new Set();
  ...
  return { url: location.href, btns, inputs, radios, texts: Array.from(set).slice(0, 70) };
}
...
(async () => {
  ...
  await freshen(page); // if banner appeared, form values may be lost after reload — re-check
  ...
  console.log('=== customer info step ===');
  console.log(JSON.stringify(await page.evaluate(dump), null, 1));
  await page.screenshot({ path: '/tmp/step2_customer.png', fullPage: true });
  await browser.close();
})();
